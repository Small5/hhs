		...
		/*$(function(){
			$.get("getEvallevel.do",
	                {"year":document.getElementById('yearid').value},function(data){
	                	console.log("好嗨哟"+data[0]);
	                });
			country.eachLayer(function (layer) {
				console.log(layer.feature.properties.Name_CN);
				if(layer.feature.properties.Name_CN=="额济纳旗"){
					layer.bindPopup('<div id="marker1" class="echarts"></div>').openPopup();
				}else if(layer.feature.properties.Name_CN=="嘉峪关市"){
					layer.bindPopup('<div id="marker2" class="echarts"></div>').openPopup();
				}
	    	...
	    	$.get("getEvallevel.do",
	                {"year":document.getElementById('yearid').value},function(data){
	    				...
	    				country.eachLayer(function (layer) {
	    					if(layer.feature.properties.Name_CN=="额济纳旗"){   
	    						//console.log("${ejinaqi}");
	    						layer.bindPopup("额济纳旗",{}).openPopup();
